...
let instructions = [
  "inc": { (register: String, diff: Int) -> Void in
      registers[register, default: 0] += diff
    },
  "dec": { (register: String, diff: Int) -> Void in
      registers[register, default: 0] -= diff
    }
  ]

let conditions = [
  ">": { (register: String, comp: Int) -> Bool
    in registers[register, default: 0] > comp },
  "<": { (register: String, comp: Int) -> Bool
    in registers[register, default: 0] < comp },
  "==": { (register: String, comp: Int) -> Bool
    in registers[register, default: 0] == comp },
...
for line in input.split(separator: "\n") {
  ...
  let register = String(splits[0]),
    instruction = String(splits[1]),
    diff = Int(splits[2])!,
    conditionRegister = String(splits[4]),
    condition = String(splits[5]),
    condidtionNumber = Int(splits[6])!
  if conditions[condition]!(conditionRegister, condidtionNumber) {
    instructions[instruction]!(register, diff)
  }
  if registers[register, default: 0] > maxReg {
    maxReg = registers[register, default: 0]
  }
}
